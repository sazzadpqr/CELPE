...
export default function PaywallCmsPage() {
  ...
  useEffect(() => {
    fetch(getApiUrl("/api/admin/paywall-cms"), { headers: getAuthHeader() })
      .then((r) => r.json())
      .then((d: PaywallCms) => setCms({ ...DEFAULTS, ...d }))
  ...
  const save = async () => {
    setSaving(true);
    try {
      const r = await fetch(getApiUrl("/api/admin/paywall-cms"), {
        method: "PUT",
  ...
  const Field = ({ field, label, placeholder, multiline }: { field: keyof PaywallCms; label: string; placeholder?: string; multiline?: boolean }) => (
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-mono text-sm">Cabeçalho</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <Field field="yearlyLabel" label="Label (preço/mês + economia)" placeholder="R$ 39,99/mês — economize 11%" />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-mono text-sm">Benefícios (lista de funcionalidades)</CardTitle>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-mono text-sm">Rodapé / Nota Legal</CardTitle>
            </CardHeader>
            <CardContent>
